...
// Evaluate at runtime, but usually constant-folded by GCC:
static inline const char* doodle_filename(const char* file)
{
    const char* slash = __builtin_strrchr(file, '/');
    return (slash ? slash + 1 : file);
}
#define DOODLE_FILENAME doodle_filename(__FILE__)
#else
...
#define DOODLE_APP_LOG_ERROR(...) \
    printf(DOODLE_ERROR_COLOR DOODLE_APP_MSG_PREFIX " [%s:%d] [ERROR] " DOODLE_SUFFIX_COLOR __VA_ARGS__, \
           DOODLE_FILENAME, DOODLE_LINE)
#else
